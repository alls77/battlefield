...
class Vehicle(Unit):
    ...
    def total_health(self):
        return mean([operator.health for operator in self.__active_operators]) + self.health

    @property
    def is_active(self):
        return self.health > 0 and any(self.__active_operators)

    def inflict_damage(self):
        return 0.5 + sum(operator.experience for operator in self.__active_operators)

    def get_damage(self, damage):
        self._health = max(0, self._health - (damage * 0.6))
        random_operator = random.choice(self.__active_operators)

        if self.health == 0:
            return
        elif len(self.__active_operators) > 1:
            for operator in self.__active_operators:
                if operator is random_operator:
                    operator.get_damage(damage * 0.2)
                else:
                    operator.get_damage((damage * 0.2) / (len(self.__active_operators) - 1))
        else:
